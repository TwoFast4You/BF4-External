...
void Cheat::SpectatorList()
{
    ...
    {
        ImGui::Text("[%d] %s", j, NameList[j].c_str());
    }
...
void Cheat::Menu()
{
    ...
    ImGui::Begin("Battlefield 4 [ EXTERNAL ]", &ShowMenu, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
    ...
    case 0:
        ...
        ImGui::NewLine();
        ImGui::SeparatorText("Skeleton Config");
        ImGui::SliderFloat("Alpha", &color_bone.w, 0.f, 1.f);
        ImGui::SliderFloat("LineSize", &BoneLine, 1.f, 3.f);
...
void Cheat::ESP()
{
    ...
    uint64_t LocalVehicle = m.RPM<uint64_t>(LocalPlayer + offset::ClientVehicle);
    ...
    if (LocalVehicle)
    {
        uint64_t LocalDynamicPhysicsEntity = m.RPM<uint64_t>(LocalVehicle + 0x238);

        if (LocalDynamicPhysicsEntity)
        {
            uint64_t pPhysicsEntity = m.RPM<uint64_t>(LocalDynamicPhysicsEntity + 0xA0); // EntityTransform
            Matrix LocalVehicleTranfsorm = m.RPM<Matrix>(pPhysicsEntity + 0x0); // Transform

            LocalPosition = Vector3(LocalVehicleTranfsorm._41, LocalVehicleTranfsorm._42, LocalVehicleTranfsorm._43);
        }
    ...
    for (int i = 0; i < 70; i++)
    {
        ...
        if (spect)
        {
            ...
            char pName[64];
            ReadProcessMemory(m.pHandle, (void*)(ClientPlayer + 0x1836), &pName, sizeof(pName), NULL);
        ...
        if (ScreenPosition != Vector2(0.f, 0.f))
        {
            ...
            if (v_Box)
            {
                ...
                case 0: // 2D
                    ...
                    break;
                case 1: // 3D
                    DrawAABB(GetAABB(ClientSoldier), PlayerPosition, Yaw, color);
                    break;
                default:
                    break;
                }
                ...
                for (int i = 0; i < 12; ++i)
                {
                    ...
                    if (WorldToScreen(Bone1, Out1) && WorldToScreen(Bone2, Out2))
                    {
                        ...
                        if (Out1 != Vector2(0.f, 0.f) || Out2 != Vector2(0.f, 0.f))
                            DrawLine(ImVec2(Out1.x, Out1.y), ImVec2(Out2.x, Out2.y), color_bone, (int)BoneLine);
                    }  
                }
            }
            ...
            // Health Bar
            if (v_Health)
                HealthBar(ScreenPosition.x - Width - 5, ScreenPosition.y + 1, 2.f, -Height - 1, Health, 100.f, true);
...
void Cheat::FuncMisc()
{
    ...
    if (SwayEditor && Alive)
    {
        ...
        if (weaponPtr && m.RPM<float>(weaponPtr + 0x430) != SwayValue)
        {
...
void Cheat::DrawAABB(AxisAlignedBox aabb, Vector3 m_Position, float Yaw, ImColor color)
{
    ...
    DrawLine(ImVec2(fld.x, fld.y), ImVec2(brt.x, brt.y), color, 1.f);
    DrawLine(ImVec2(brb.x, brb.y), ImVec2(blt.x, blt.y), color, 1.f);
    DrawLine(ImVec2(fld.x, fld.y), ImVec2(brb.x, brb.y), color, 1.f);
    DrawLine(ImVec2(brt.x, brt.y), ImVec2(blt.x, blt.y), color, 1.f);
    DrawLine(ImVec2(frt.x, frt.y), ImVec2(bld.x, bld.y), color, 1.f);
    DrawLine(ImVec2(frd.x, frd.y), ImVec2(flt.x, flt.y), color, 1.f);
    DrawLine(ImVec2(frt.x, frt.y), ImVec2(frd.x, frd.y), color, 1.f);
    DrawLine(ImVec2(bld.x, bld.y), ImVec2(flt.x, flt.y), color, 1.f);
    DrawLine(ImVec2(frt.x, frt.y), ImVec2(fld.x, fld.y), color, 1.f);
    DrawLine(ImVec2(frd.x, frd.y), ImVec2(brb.x, brb.y), color, 1.f);
    DrawLine(ImVec2(brt.x, brt.y), ImVec2(bld.x, bld.y), color, 1.f);
    DrawLine(ImVec2(blt.x, blt.y), ImVec2(flt.x, flt.y), color, 1.f);
}
