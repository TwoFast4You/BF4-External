...
void Core::ESP()
{
    ...
    for (int i = 0; i < 70; i++)
    {
        ...
        if (v_VehicleESP && pEntity->InVehicle())
        {
            ...
            ImColor VehicleColor = pEntity->Team == pLocal->Team ? color_Team : color_Normal;

            // Get some size.
            DrawAABB(pEntity->VehicleAABB, pEntity->VehicleTranfsorm, VehicleColor);
        ...
        if (ScreenPosition != Vector2(0.f, 0.f))
        {
            ...
            if (v_Bone)
            {
                ...
                for (int i = 0; i < 12; ++i)
                {
                    Vector3 Bone1 = GetBone(pEntity->ClientSoldier, aSkeleton[i][0]);
                    Vector3 Bone2 = GetBone(pEntity->ClientSoldier, aSkeleton[i][1]);
