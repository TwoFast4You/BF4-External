...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE hInstPrev, PSTR cmdline, int cmdshow)
{
    ...
    WNDCLASSEXA wc = { sizeof(WNDCLASSEXA), 0, WndProc, 0, 0, NULL, NULL, NULL, NULL, "PL", "NULL", NULL };
    RegisterClassExA(&wc);
...
void CThread()
{
    ...
    while (Run)
    {
        ...
        DWORD Flag = NULL;
        GetWindowDisplayAffinity(OverlayHwnd, &Flag);

        if (StreamProof && Flag == WDA_NONE)
            SetWindowDisplayAffinity(OverlayHwnd, WDA_EXCLUDEFROMCAPTURE);
        else if (!StreamProof && Flag == WDA_EXCLUDEFROMCAPTURE)
