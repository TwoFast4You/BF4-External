...
void Cheat::RenderMenu()
{
    ...
    if (ImGui::BeginTabBar("##ContextTabBar"))
    {
        ...
        if (ImGui::BeginTabItem("    Misc    "))
        {
            ...
            ImGui::Checkbox("SpectatorList", &cfg.SpectatorList);
            ...
            ImGui::Checkbox("UnlockAll", &cfg.UnlockAll);
            ImGui::EndChild();
            /*---------------*/
            ImGui::SameLine();
            /*---------------*/
...
void Cheat::RenderESP()
{
    ...
    for (int i = 0; i < 72; i++)
    {
        ...
        if (ScreenPosition != Vector2(0.f, 0.f))
        {
            ...
            {
                std::string text = std::to_string((int)distance) + "m";
                String(ImVec2(ScreenPosition.x - (ImGui::CalcTextSize(text.c_str()).x / 2.f), ScreenPosition.y), ImColor(1.f, 1.f, 1.f, 1.f), text.c_str());
            }
            ...
            if (cfg.AimBot)
            {
                if (!IsKeyDown(cfg.AimKey0) && !IsKeyDown(cfg.AimKey1))
                    ...
                    continue;
                else if (cfg.Aim_MaxDistance < distance)
                    continue;
                else if (cfg.VisCheck && !pEntity->IsVisible())
                    continue;
                ...
                if (AimReady)
                {
                    ...
                    if (!WorldToScreen(pEntity->GetBone(cfg.AimTargetBone == 0 ? 104 : 6), TargetScreen))
                        continue;
...
void Cheat::RenderSpectatorList()
{
    ...
    if (first)
    {
        ImGui::SetNextWindowPos(ImVec2(8, 24));
        ImGui::SetNextWindowSize(ImVec2(225, 115));
        first = false;
    }

    ImGui::Begin("SpectatorList", (bool*)NULL, ImGuiWindowFlags_NoScrollbar);
