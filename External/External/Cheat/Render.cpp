...
void Cheat::RenderMenu()
{
    ...
    if (ImGui::BeginTabBar("##ContextTabBar"))
    {
        ...
        if (ImGui::BeginTabItem("   Visual   "))
        {
            ...
            ImGui::Checkbox("Vehicle ESP", &cfg.ESP_Vehicle);
...
void Cheat::RenderESP()
{
    ...
    for (int i = 0; i < 72; i++)
    {
        ...
        // Vehicle ESP
        if (cfg.ESP_Vehicle && pEntity->InVehicle())
        {
                ...
                for (int j = 0; j < 12; ++j)
                {
                    ...
                    if (WorldToScreen(Bone1, Out1) && WorldToScreen(Bone2, Out2))
                    {
                        ...
                        if (Out1 != Vector2(0.f, 0.f) || Out2 != Vector2(0.f, 0.f))
                        {
                            ...
                            DrawLine(ImVec2(Out1.x, Out1.y), ImVec2(Out2.x, Out2.y), ImColor(1.f, 1.f, 1.f, 1.f), 1);
                        }
